...
class ServicioReporteController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware('can:subir fotos reportes de servicios')->only([
            'storeFoto',
        ]);

        $this->middleware('can:eliminar fotos reportes de servicios')->only([
            'destroyFoto',
        ]);
    }
    ...
    private function validationRules(bool $includeFotos = true): array
    {
        $rules = [
        ...
        if ($includeFotos) {
            ...
            $rules['fotos.*'] = ['nullable', 'image', 'mimes:jpg,jpeg,png,webp', 'max:5120'];
            $rules['descripcion'] = ['nullable', 'array'];
    ...
    private function buildWhatsappText(Servicio $servicio, ServicioReporte $reporte): string
    {
        ...
        $fecha = $reporte->fecha ? date('d/m/Y', strtotime($reporte->fecha)) : null;
    ...
    public function whatsapp(Servicio $servicio, ServicioReporte $reporte)
    {
        ...
        return response()->json([
            ...
            'url' => 'https://wa.me/?text=' . urlencode($reporte->whatsapp_texto),
        ]);
    }
    ...
    public function storeFoto(Request $request, Servicio $servicio, ServicioReporte $reporte)
    {
        abort_if($reporte->servicio_id !== $servicio->id, 404);
    ...
    public function destroyFoto(Servicio $servicio, ServicioReporte $reporte, ServicioReporteFoto $foto)
    {
        abort_if($reporte->servicio_id !== $servicio->id, 404);
