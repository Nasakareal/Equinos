...
class PuestaDisposicionController extends Controller
{
    public function index(Request $request)
    {
        ...
        $puestas = $query->paginate(20);

        $personals = Personal::query()->orderBy('nombres')->get(['id', 'nombres', 'grado', 'cargo']);

        return view('puestas_disposicion.index', compact('puestas', 'personals'));
    }
    ...
    public function create(Request $request)
    {
        ...
        return view('puestas_disposicion.create', compact('personals', 'anio'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('puestas_disposicion.index')->with('success', 'Puesta a disposición registrada correctamente');
    }

    public function show(PuestaDisposicion $puesta_disposicion)
    {
        $puesta_disposicion->load(['personal']);
        return view('puestas_disposicion.show', ['pd' => $puesta_disposicion]);
    }

    public function edit(PuestaDisposicion $puesta_disposicion)
    {
        $personals = Personal::query()->orderBy('nombres')->get(['id', 'nombres', 'grado', 'cargo']);

        return view('puestas_disposicion.edit', ['pd' => $puesta_disposicion, 'personals' => $personals]);
    }
    ...
    public function update(Request $request, PuestaDisposicion $puesta_disposicion)
    {
        ...
        return redirect()->route('puestas_disposicion.index')->with('success', 'Puesta a disposición actualizada correctamente');
    }
    ...
    public function destroy(PuestaDisposicion $puesta_disposicion)
    {
        ...
        return redirect()->route('puestas_disposicion.index')->with('success', 'Puesta a disposición eliminada correctamente');
    }
}
