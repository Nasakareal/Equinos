...
<div class="card mb-3">
    <div class="card-body d-flex align-items-center" style="gap:10px;">
        <form method="GET" action="{{ url('/servicios') }}" class="d-flex" style="gap:10px;">
            <input type="date" name="fecha" value="{{ $fechaSeleccionada }}" class="form-control">
            <button class="btn btn-primary">Filtrar</button>
        </form>

        <a href="{{ url('/servicios?fecha=' . now()->toDateString()) }}" class="btn btn-secondary">
            Hoy
        </a>
    </div>
</div>
                ...
                <tbody>
                    @foreach ($servicios as $index => $servicio)
                        <tr>
                            ...
                            <td>{{ $servicio->patrulla->nombre ?? '-' }}</td>
                            <td>{{ $servicio->user->name ?? '-' }}</td>

                            <td>
                                <a href="{{ url('/servicios/' . $servicio->id) }}" class="btn btn-info btn-sm">
                                    <i class="fa fa-eye"></i>
                                </a>

                                <a href="{{ url('/servicios/' . $servicio->id . '/edit') }}" class="btn btn-success btn-sm">
                                    <i class="fa fa-pen"></i>
                                </a>
                            </td>
                        </tr>
