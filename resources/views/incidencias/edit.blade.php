...
<div class="row">
    <div class="col-md-12">
        <div class="card card-outline card-primary">
            ...
            <div class="card-body">
                <form action="{{ route('incidencias.update', $incidence->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="form-group">
                                ...
                                <select name="personal_id"
                                    ...
                                    @foreach ($personals as $p)
                                        <option value="{{ $p->id }}"
                                            {{ (string)old('personal_id', $incidence->personal_id) === (string)$p->id ? 'selected' : '' }}>
                                            {{ $p->nombres }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select name="incidence_type_id"
                                    ...
                                    @foreach ($incidence_types as $type)
                                        <option value="{{ $type->id }}"
                                            {{ (string)old('incidence_type_id', $incidence->incidence_type_id) === (string)$type->id ? 'selected' : '' }}>
                                            {{ $type->nombre }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="date"
                                       ...
                                       value="{{ old('fecha_inicio', optional($incidence->fecha_inicio)->format('Y-m-d')) }}"
                                       required>
                                @error('fecha_inicio')
                                    <span class="invalid-feedback"><strong>{{ $message }}</strong></span>
                                @enderror
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="date"
                                       ...
                                       value="{{ old('fecha_fin', optional($incidence->fecha_fin)->format('Y-m-d')) }}">
                                @error('fecha_fin')
                                    <span class="invalid-feedback"><strong>{{ $message }}</strong></span>
                                @enderror
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <textarea name="comentario"
                                          ...
                                          placeholder="Detalles de la incidencia...">{{ old('comentario', $incidence->comentario) }}</textarea>
                                @error('comentario')
                                    <span class="invalid-feedback"><strong>{{ $message }}</strong></span>
                                @enderror
                            </div>
                    ...
                    <a href="{{ route('incidencias.show', $incidence->id) }}" class="btn btn-secondary">
